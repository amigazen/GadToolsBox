...
#define LIST_ColumnsPos (LIST_Dummy+17)
#define LIST_HookSub (LIST_Dummy+18)
#define LIST_HookEdit (LIST_Dummy+19)
#define LIST_HookDraw (LIST_Dummy+20)

/* Hook callback message IDs */
...
#define TLVCB_OK 0
#define TLVCB_UNKNOWN 1

/* Hook message structures */
struct TLVSubMsg
{
	ULONG tlvm_MethodID;
	struct LVNode *tlvm_Node;
};
...
struct TLVDrawMsg
{
	ULONG tlvm_MethodID;
	struct LVNode *tlvm_Node;
	struct RastPort *tlvm_RPort;
	struct DrawInfo *tlvm_DrawInfo;
	struct Rectangle tlvm_Rect;
	UWORD *tlvm_ColPos;
};
